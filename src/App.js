...
function App() {
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem('Database')){
       ...
       const database =  [{id: 1, nombre: 'Editor del Sitio', logo:'gear', enlace:'/', contenido:{logo:'https://res.cloudinary.com/dfxwg2qk7/image/upload/v1676267925/ImagesRestaurant/cpphdnmktzabevrrm1yg.png',titulo:'100% Pura Carne', cabecera:'CARNE DE LA MEJOR CALIDAD', portada :'https://res.cloudinary.com/dfxwg2qk7/image/upload/q_20/v1676289008/ImagesRestaurant/fwcbdrgxurl0whst4orl.jpg',acercade:'Ut enim ad minim veniam perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae.',contacto:{Nombre:'El Rico Agucho',telefono:'922 692 354',pais:'Perú',ciudad:'El Agustino',calleNum:' Pjs cahuide N300',correo:'jinfanzonquispe30@gmail.com', redes:{facebook:'',tweeter:'',instagram:''}} }},
                           {id: 2, nombre: 'Menu', logo:'journal', enlace:'/Menu/', categoria : [{id: 'cat1', nombre:'Principales', enlace:'/Menu/Principales', contenido:[
                                                                           ...
                                                                           {id: 'cat2', nombre:'Bebidas', enlace:'/Menu/Bebidas',contenido :[
                                                                                                                                                   ...
                                                                                                                                                   {id:'beb3',nombre:'Coca Cola',imagen :'https://thefoodtech.com/wp-content/uploads/2022/06/industria_de_bebidas_1.jpg',tamaño:'grande',recomendado : false, oferta:false,precio :10},
     ...
     const [textoPrincipal, setTextoPrincipal] = useState(database[0].contenido.titulo);
     const [cabecera, setCabecera] = useState(database[0].contenido.cabecera)
     const [imageUrl, setImageUrl] = useState(database[0].contenido.portada);
     const [acercade,setAcercade] = useState(database[0].contenido.acercade);
     const [nombre,setNombre] = useState(database[0].contenido.contacto.Nombre);
     const [pais,setPais] = useState(database[0].contenido.contacto.pais);
     const [numero,setNumero] = useState(database[0].contenido.contacto.telefono);
     const [ciudad,setCiudad] = useState(database[0].contenido.contacto.ciudad);
     const [calleNum,setCalleNum] = useState(database[0].contenido.contacto.calleNum);
     const [correo,setCorreo] = useState(database[0].contenido.contacto.correo);
